...
const DetailedViewCard = (props) => {
    ...
    const {data: comments, isLoadingComments, errorComments}  = useFetch(`http://localhost:5000/community/${props.post._id}/comments`);
    const {data: owner, isLoadingOwner, errorOwner}  = useFetch('http://localhost:5000/users/' + props.post.owner);
    ...
    const deletePost = () => {
        fetch(`http://localhost:5000/community/${props.post._id}`, {
            method: 'DELETE',
        ...
        .catch(err => {
            ...
            var alertMessage = ''
            if (err === 404){
                alertMessage = 'This social media post does not exist'
            } else if (err === 401){
                alertMessage = 'You must be logged in to do that'
            } else if (err === 403) {
                alertMessage = 'You must be the owner of this social media post to do that'
            } else {
    ...
    function attemptComment() {
        if (!isLoggedIn.loggedIn){
            navigate('/logIn', { state: "You must be logged in to post comments!" })
            return
        }
    }
    ...
    return (  
        <Card bg="light" className="my-4 shadow border-0 rounded-3 w-75" key={props.post._id}>
            ...
            <div className="d-flex flex-row justify-content-center">
                    {(isLoggedIn.loggedIn && props.post.likes.includes(isLoggedIn.user._id)) ? 
                        <div className="d-flex flex-row justify-content-center">
                    ...
                    {(isLoggedIn.loggedIn && props.post.dislikes.includes(isLoggedIn.user._id)) ? 
                        <div className="d-flex flex-row justify-content-center">
            ...
            <Form className="d-flex mt-4 mb-5 flex-column justify-content-center align-items-center" id="comment-section" onClick={attemptComment}>
            ...
            {comments && 
                <div className="mb-4">
                    {comments.map((comment) => (
                        <CommunityComment key={comment.id} type={'parent'} comment={comment} post={props.post}></CommunityComment>
                    ))}
                </div>
            }
        </Card>
